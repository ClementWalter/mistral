...
By definition, we call the standard input space the case where $\mathbf{X}$ is a standard Gaussian vector, ie. a vector with independent standard Gaussian coordinates. **All the stochastic methods are developed for standard Gaussian input space.** In other words, when the problem at hand does not use only independent standard Gaussian random variables, an iso-probabilistic transformation has to be done before a call to the code `g`, often called the limit-state function `lsf`.
...
### Defining a proper limit-state function $g$
All the methods implemented in `mistral`  require that $g$ be a function taking a matrix as input and returning a vector as ouput. Indeed, in order to allow for parallel computing, batches of points to be evaluated are given as a matrix of column vectors. Hence, depending on the processing capaiblities and/or the implementation, computation of the model $g$ on the points $\mathbf{X}$ can be made a lot faster.
...
lsf(x)
```
obviously does not provide the expected result. There are indeed several possibilities to make the example fit into this framework. In this simple case, one has an analytical expression of the function. This can arise when using the package for educational/illustrative purpose. An easy way to fix that is then to redefine the function with `x` as a matrix:
```{r}
lsf <- function(x) {
  x[1,] + x[2,]
}
lsf(x)
```
Note here that the function considers each **column** as a point. This is to be consistent with the default behaviour of `as.matrix` function:
```{r}
x <- 1:2
as.matrix(x)
```
...
In general (practical) settings, no analytique expression is available. Let us denote by `myCode` the given computer code. `myCode` is supposed to be able to be called onto a vector. Then the `apply` function can be used:
```{r}
lsf <- function(x){
  x <- as.matrix(x)
  apply(x, 2, myCode)
...
The initialisation of an MPI backend is rather similar to the one the SNOW backend:
```r
...
Y_{n+1} \sim Y | Y > Y_n
$$
In other words, each element is generated conditionally greater than the previous one. This random walk is a Poisson process and lets build true counterparts of the crude Monte Carlo estimators of a probability, a quantile or the _cdf_ of $Y$. 
...
```
This latter estimation also enables parallel computation but in a 2-step algorithm. The empirical _cdf_ is estimated until the farthest _state_ reached by the iid random walks. Note also that the condifence interval requires to run the algorithm a little but longer and thus is optional: default parameter `compute_confidence = FALSE`.
```{r}
mp <- mistral::MP(dimension = 2,  lsf = mistral::kiureghian, p = mp$p, N = 1e2,
...
predict(svm.model, data.frame(x1=rnorm(1), x2=rnorm(1)))
```
...
In this setting, all the statistical and/or metamodel-based algorithms have proven in some cases good efficiency. Amongst them, the `Moving Particles` and `Bayesian Moving Particles` are the only one not only focusing on the given threshold but delivering an uncertainty quantification for the random output _until_ this threshold.
