...
MetaIS = function(dimension,
  ...
  if(N_DOE>0){
    ...
    if(is.null(learn_db)){
      ...
      learn_db = cbind(learn_db,DoE); dimnames(learn_db) <- list(rep(c('x', 'y'), length.out = dimension), NULL);
  ...
  while(!criterion) {
    ...
    if(niter>Niter_seed && n<N_seeds){
      ...
      if(verbose>0){cat(" * Add points to the learning database\n")}
      learn_db = cbind(learn_db,candidate); dimnames(learn_db) <- list(rep(c('x', 'y'), length.out = dimension), NULL);
      ...
      if(compute.PPP){
        capture.output({
          bmp <- BMP(dimension = dimension, lsf = lsf, q = failure, N = 100, N.iter = 0, learn_db = learn_db, y = lsf_value)
        })
        J <- c(J, bmp$h)
        I <- c(I, bmp$I)
        cat(" * Current alpha estimate =", bmp$alpha, "\n")
  ...
  # inDB = which(duplicated(t(cbind(U, learn_db)))) - dim(U)[2]
